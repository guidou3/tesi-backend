...
import org.processmining.plugins.balancedconformance.BalancedReplayResultProcessor;
import org.processmining.plugins.balancedconformance.DataAlignmentTraceProcessor;
...
import org.processmining.plugins.balancedconformance.controlflow.adapter.*;
...
import org.processmining.plugins.balancedconformance.dataflow.datastate.*;
...
import org.processmining.plugins.balancedconformance.result.*;
...
public class BalancedDataConformance{
...
//    public static BalancedProcessorConfiguration queryConfiguration(BalancedProcessorConfiguration config, DataPetriNet net) {
//
...
//
//            if (!hasGuards(net)) {
//                System.out.println("Variables/guards missing");
//                System.out.println("Selected DPN-net does not define variables/guards. Alignment will not consider data!");
...
//    }
    ...
    public static boolean hasGuards(DataPetriNet net) {
    ...
    protected BalancedReplayResult doBalancedDataConformanceChecking(final DataPetriNet net, final Collection<XTrace> traces,
        ...
        return doBalancedDataConformanceChecking(net, log, progressListener, config);
    }

    protected BalancedReplayResult doBalancedDataConformanceChecking(final DataPetriNet net, final XLog log,
                                                                     final Progress progressListener, final BalancedProcessorConfiguration config)
            throws ControlFlowAlignmentException, DataAlignmentException {
    ...
    private List<Future<BalancedDataAlignmentState>> computeAlignments(ExecutorService pool,
        ...
        Iterator<Entry<GroupedTraces.GroupedTrace>> groupIter = groupedTraces.asMultiset().entrySet().iterator();
        int traceIndex = 0;
        while (groupIter.hasNext()) {
            Entry<GroupedTraces.GroupedTrace> groupedTrace = groupIter.next();
        ...
        for (int futureIndex = 0; futureIndex < dataAlignmentFutures.size(); futureIndex++) {
            ...
            try {
                ...
                if (futureState.isDone()) {
                    try {
                        BalancedDataAlignmentState realState = futureState.get();
                        if (realState == null) {
    ...
    private static Int2IntMap createGroupIndexMap(GroupedTraces groupedTraces) {
        ...
        for (Entry<GroupedTraces.GroupedTrace> group : groupedTraces.asMultiset().entrySet()) {
        ...
        switch (config.getSearchMethod()) {
            case DIJKSTRA :
                controlFlowAlignmentAdapter = new AlignmentDijkstraImpl(log, net, logMapping, cfConfig,
                        BalancedDataConformance.<DijkstraTail> getStorageHandlerFactory(
            ...
            case ASTAR_GRAPH :
                controlFlowAlignmentAdapter = new AlignmentAdapterILPGraphImpl(log, net, logMapping, cfConfig,
                        BalancedDataConformance.<PILPTail> getStorageHandlerFactory(
                                config.getControlFlowStorageHandler(), config.getDataStateStorageHandler(),
                                dataAlignmentAdapter), config.isKeepControlFlowSearchSpace(),
                        config.isKeepDataFlowSearchSpace(), observer, dataAlignmentAdapter);
                break;
                ...
                switch (config.getIlpSolver()) {
                    case ILP_GUROBI :
                        return new DataAlignmentAdapter.GurobiDataAlignmentAdapter(logMapping, dataStateFactory,
                                ...
                                (config.getSearchMethod() != SearchMethod.ASTAR_TREE));
                    case ILP_LPSOLVE :
                        return new DataAlignmentAdapter.LpSolveDataAlignmentAdapter(logMapping, dataStateFactory,
                                config.getConcurrentThreads(), config.getCostDelta(),
                                config.getIncludeVirtualVariablesInTrace(),
                                (config.getSearchMethod() == SearchMethod.ASTAR_GRAPH));
                    default :
...
//        return doBalancedDataConformanceChecking(convertToDPN(net, context), log, context.getProgress(), config);
//    }B
    public BalancedReplayResult balancedAlignmentPluginHeadless(final PluginContext context, DataPetriNet dpn, XLog log,
                                                                BalancedProcessorConfiguration config) throws ControlFlowAlignmentException, DataAlignmentException {
        return doBalancedDataConformanceChecking(dpn, log, context.getProgress(), config);
    }

//    public BalancedReplayResult balancedAlignmentPluginHeadless(final PluginContext context, DataPetriNet dpn, XLog log)
//            throws ControlFlowAlignmentException, DataAlignmentException {
//        BalancedProcessorConfiguration config = BalancedDataConformance.queryConfiguration((UIPluginContext) context, dpn, log);
//        return doBalancedDataConformanceChecking(dpn, log, context.getProgress(), config);
//    }

//    public BalancedReplayResult balancedAlignmentPlugin(final UIPluginContext context, Configs configs, Petrinet net, XLog log, TransEvClassMapping activityMapping, FinalVariableMapping variableMapping) {
//        DataPetriNet dpnNet = convertToDPN(net, context);
//        return balancedAlignmentPlugin(context, configs, dpnNet, log, activityMapping, variableMapping);
//    }
//
//    public BalancedReplayResult balancedAlignmentPlugin(final UIPluginContext context, Configs configs, DataPetriNet dpnNet, XLog log, TransEvClassMapping activityMapping, FinalVariableMapping variableMapping) {
//
//        BalancedProcessorConfiguration config = queryConfiguration(context, configs, dpnNet, log, activityMapping, variableMapping);
//        try {
//            BalancedReplayResult result = doBalancedDataConformanceChecking(dpnNet, log,
