...
public final class XAlignmentExtension extends XExtension {
    ...
    private XAlignmentExtension() {
        ...
        // maybe put a model ID on log level
        // maybe put a log ID on log level

        this.traceAttributes.add((XAttribute) ATTR_FITNESS.clone()); // maybe put into quality measure extension
        this.traceAttributes.add((XAttribute) ATTR_PENALTY_COST.clone()); // maybe put into quality measure extension
        this.traceAttributes.add((XAttribute) ATTR_LIKELIHOOD_COST.clone());

        this.eventAttributes.add((XAttribute) ATTR_PENALTY_COST.clone()); // maybe put into quality measure extension
        ...
        this.eventAttributes.add((XAttribute) ATTR_ACTIVITY_ID.clone()); // maybe rename to MODEL_ID
        this.eventAttributes.add((XAttribute) ATTR_EVENTCLASS_ID.clone()); // maybe rename to LOG_ID
    ...
    public void assignMoveType(XEvent event, MoveType moveType) {
        putAttribute(event, factory.createAttributeDiscrete(KEY_MOVE_TYPE, moveType.ordinal(), this));
    }

    public void assignModelMove(XEvent event, String modelMove) {
        putAttribute(event, factory.createAttributeLiteral(KEY_MOVE_MODEL, modelMove, this));
    }

    public void assignLogMove(XEvent event, String logMove) {
        putAttribute(event, factory.createAttributeLiteral(KEY_MOVE_LOG, logMove, this));
    }

    public void assignActivityId(XEvent event, String activityId) {
        putAttribute(event, factory.createAttributeLiteral(KEY_ACTIVITY_ID, activityId, this));
    }

    public void assignEventClassId(XEvent event, String eventClassId) {
        putAttribute(event, factory.createAttributeLiteral(KEY_EVENTCLASS_ID, eventClassId, this));
    }

    private static void putAttribute(XAttributable t, XAttribute a) {
        t.getAttributes().put(a.getKey(), a);
    }
    ...
    private final static class XAlignmentMoveIterator implements Iterator<XAlignmentMove> {
        ...
        public boolean hasNext() {
            return traceIter.hasNext();
        }

        public XAlignmentMove next() {
            return new XAlignmentMoveImplementation(traceIter.next());
        }
    ...
    private static final class XAlignmentMoveImplementation implements XAlignmentMove {
        ...
        public String toString() {
            String modelMove = getModelMove();
            if (modelMove != null) {
                return modelMove + "(" + getType() + ")";
            } else {
                return getLogMove() + "(" + getType() + ")";
            }
        }
    ...
    private final static class XAlignmentIterator implements Iterator<XAlignment> {
        ...
        public boolean hasNext() {
            return logIter.hasNext();
        }

        public XAlignment next() {
            return new XAlignmentImplementation(logIter.next());
        }
    ...
    private static final class XAlignmentImplementation extends AbstractList<XAlignmentMove> implements XAlignment {
        ...
        public Iterator<XAlignmentMove> iterator() {
            return new XAlignmentMoveIterator(trace);
        }
        ...
        public int hashCode() {
            ...
            result = prime * result + (doubleHashCode(getFitness()));
            return result;
        }
        ...
        public String toString() {
            ...
            Iterator<XAlignmentMove> iterator = iterator();
